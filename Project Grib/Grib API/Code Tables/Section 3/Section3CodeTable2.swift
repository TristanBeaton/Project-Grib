//
//  Section3CodeTable2.swift
//  Project Grib
//
//  Created by Tristan Beaton on 19/09/17.
//  Copyright © 2017 Tristan Beaton. All rights reserved.
//

import Foundation

enum Section3CodeTable2 {
    
    case SphericalRadius6367470
    
    case SphericalRadiusSpecified
    
    case OblateSpheroidIAU
    
    case OblateSpheroidSpecifiedKilometers
    
    case OblateSpheroidIAGGRS80
    
    case WGS84
    
    case SphericalRadius6371229
    
    case OblateSpheroidSpecifiedMeters
    
    case SphericalRadius6371200WGS84
    
    case Airy1830Spheroid
    
    case Reserved
    
    case ReservedForLocalUse
    
    case Missing
    
    init(_ value:UInt8) {
        
        if value == 0 { self = .SphericalRadius6367470; return }
        
        if value == 1 { self = .SphericalRadiusSpecified; return }
        
        if value == 2 { self = .OblateSpheroidIAU; return }
        
        if value == 3 { self = .OblateSpheroidSpecifiedKilometers; return }
        
        if value == 4 { self = .OblateSpheroidIAGGRS80; return }
        
        if value == 5 { self = .WGS84; return }
        
        if value == 6 { self = .SphericalRadius6371229; return }
        
        if value == 7 { self = .OblateSpheroidSpecifiedMeters; return }
        
        if value == 8 { self = .SphericalRadius6371200WGS84; return }
        
        if value == 9 { self = .Airy1830Spheroid; return }
        
        if value >= 10 && value <= 191 { self = .Reserved; return }
        
        if value >= 192 && value <= 254 { self = .ReservedForLocalUse; return }
        
        self = .Missing
    }
}
